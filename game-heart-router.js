(function(){
  ...
  const RETIRED_BONUS_HEART_KEYS = [
    'opoong_bonus_hearts_v1',
    'opoong_bonus_hearts_seoyul_v1'
  ];
  const HEART_MAX = 50;
  ...
  function clearRetiredBonusHearts(){
    try{
      RETIRED_BONUS_HEART_KEYS.forEach(function(key){ localStorage.removeItem(key); });
    }catch(_){ }
  }
  ...
  function installHeartRouter(){
    ...
    function renderBase(){
      ...
      if(count) count.textContent = `${base} / ${api.max || HEART_MAX}`;
      if(modalCount) modalCount.textContent = `${base} / ${api.max || HEART_MAX}`;
      if(hub) hub.classList.toggle('heart-empty', base <= 0);
    }

    function spendBaseHeart(){
      const base = baseCount();
    ...
    api.get = baseCount;
    api.spend = spendBaseHeart;
    try{delete api.getBonus;}catch(_){ }
    api.render = renderBase;
  ...
  window.addEventListener('storage', function(event){
    if(event.key === HEART_KEY || RETIRED_BONUS_HEART_KEYS.includes(event.key)){
      clearRetiredBonusHearts();
      try{window.OPOONG_GAME_HEARTS?.render?.();}catch(_){ }
    }
  });
